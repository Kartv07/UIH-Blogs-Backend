...
ParentCategoryrouter.put("/upate/:id", async (req, res) => {
  try {
    ...
    if(!req?.params?.id) {  
      return res.status(404).json({ message: "Please provide parent category id !" });
      }

    const updatedObj = req.body;

    let existAndUpdate = await ParentCategoryCollection.findOneAndUpdate({_id: new mongoose.Types.ObjectId(req.params.id)}, updatedObj, {new: true});
